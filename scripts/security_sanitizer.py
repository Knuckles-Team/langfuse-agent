...
# release-build-requirements.txt is the hash-pinned, build-only input consumed by
# the owned reproducible-wheel workflow.  It is deliberately separate from the
# runtime requirements and is therefore a governed release input, not scratch.
ALLOWED_TXT_NAMES = {
    "requirements.txt",
